...
- (void) appDidEnterForeground {
    [self turnTorch:torchState];
}

#pragma mark - Public
- (void) initializeUI {
    self.flipCameraButton.layer.cornerRadius = 24;
    self.flipCameraButton.layer.backgroundColor = [[UIColor blackColor] CGColor];

    self.videoButton.layer.cornerRadius = 24;
    self.videoButton.layer.backgroundColor = [[UIColor blackColor] CGColor];

    self.micButton.layer.cornerRadius = 24;
    self.micButton.layer.backgroundColor = [[UIColor blackColor] CGColor];

    self.flashButton.layer.cornerRadius = 24;
    self.flashButton.layer.backgroundColor = [[UIColor blackColor] CGColor];

    self.disconnectButton.layer.cornerRadius = 24;
    self.disconnectButton.layer.backgroundColor = [[UIColor redColor] CGColor];
}
    ...
    if ([flashLight isTorchAvailable] && [flashLight isTorchModeSupported:AVCaptureTorchModeOn]) {
        ...
        if (success)
        {
            if (!on) {
                ...
                NSString *imagePath = [[NSBundle mainBundle] pathForResource:@"ios_flash_off" ofType:@"png"];
                [self.flashButton setImage:[UIImage imageWithContentsOfFile:imagePath] forState:UIControlStateNormal];
            } else {
                [flashLight setTorchMode:AVCaptureTorchModeOn];
                NSString *imagePath = [[NSBundle mainBundle] pathForResource:@"ios_flash_on" ofType:@"png"];
                [self.flashButton setImage:[UIImage imageWithContentsOfFile:imagePath] forState:UIControlStateNormal];
            }
...
- (IBAction)flashButtonPressed:(id)sender {
    ...
    [self turnTorch:torchState];
    if (torchState) {
        [self.view makeToast:@"Flash turned on."];
    } else {
        [self.view makeToast:@"Flash turned off."];
    }
}
...
- (IBAction)micButtonPressed:(id)sender {
    ...
    if (self.localAudioTrack) {
        ...
        if (self.localAudioTrack.isEnabled) {
            ...
            NSString *whiteImagePath = [[NSBundle mainBundle] pathForResource:@"ios_mic_on" ofType:@"png"];
            [self.micButton setImage:[UIImage imageWithContentsOfFile:whiteImagePath] forState:UIControlStateNormal];
            [self.view makeToast:@"Mic turned on."];
        } else {
            self.micButton.selected = true;
            self.micButton.alpha = self.micButton.selected ? 0.7 : 1;

            NSString *redImagePath = [[NSBundle mainBundle] pathForResource:@"ios_mic_off" ofType:@"png"];
            [self.micButton setImage:[UIImage imageWithContentsOfFile:redImagePath] forState:UIControlStateNormal];
            [self.view makeToast:@"Mic turned off."];
        }
    }
}
...
- (IBAction)videoButtonPressed:(id)sender {
    if(self.localVideoTrack){
        ...
        if(self.localVideoTrack.isEnabled){
            ...
            NSString *whiteImagePath = [[NSBundle mainBundle] pathForResource:@"ios_video_on" ofType:@"png"];
            [self.videoButton setImage:[UIImage imageWithContentsOfFile:whiteImagePath] forState:UIControlStateNormal];
            [self.view makeToast:@"Camera turned on."];
        }else {
            self.videoButton.selected=true;
            self.videoButton.alpha = self.videoButton.selected ? 0.7 : 1;
            NSString *redImagePath = [[NSBundle mainBundle] pathForResource:@"ios_video_off" ofType:@"png"];
            [self.videoButton setImage:[UIImage imageWithContentsOfFile:redImagePath] forState:UIControlStateNormal];
            [self.view makeToast:@"Camera turned off."];
        }
    }
}
...
- (void)didConnectToRoom:(TVIRoom *)room {
    ...
    [self logMessage:@"Waiting on client to join"];
...
- (void)room:(TVIRoom *)room participantDidDisconnect:(TVIRemoteParticipant *)participant {
    ...
    [self stopTimer];
    [self logMessage:@"Client disconnected"];
}
